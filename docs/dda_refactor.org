...
Mais les choses ne changent jamais en un éclair. Il faut du temps pour avancer. C'est pourquoi la démocratie s'est progressivement perfectionnée, amélioré, au fil des années, en fonction des courants de pensées et de l'évolution technologique de la société (qui accélère la diffusion des idées, et qui libère du temps pour Penser). 
...
Au fil du temps, la démocratie a été remplacé par une oligarchie que plus personne ne peut nier depuis l'investiture de Mr Macron.
...
- La télévision : permet d'envoyer de l'image et du son, en direct, sur un très large périmètre, et un grand nombre de personnes.

Notez qu'aucune de ces inventions ne permet l'interaction entre plus de 2 personnes à la fois (pour le téléphone)
La diffusion d'information reste limité dans 1 sens unique, celui qui émet et celui(ou ceux) qui reçoit.
Il n'y a qu'internet pour dépasser cette limitation, ainsi que toutes les autres (distance de diffusion, support, durée d'accessibilité, etc)
...
Le réseau communecter.org, tente d'apporter une nouvelle amélioration à internet, en ajoutant une notion géographique aux informations, de façon à géolocaliser les données pertinentes pour une zone géographique donnée, et à les rendre accessibles facilement aux habitants de ce territoire.
(C'est ce que j'appelle la géo-communication.)

Cette nouvelle amélioration, rend possible une nouvelle forme de démocratie basée sur la participation locale, et peut permettre de régler le problème de la représentativité des élus. Pour faire simple, si nous avons une oligarchie, c'est parce que nous avons des représentant. 
...
Il me semblerait absolument nécessaire d'organiser AUSSI des assemblées locales (IRL) pour parler de politique entre citoyens, pour débattre des idées, pour faire des propositions. Le système informatique sera là pour rassembler l'ensemble de ces travaux, les rendre disponibles à tous, permettre le vote pour les prises de décisions à grande échelle, etc, etc.
...
Et pas besoin d'élus pour écrire les lois.
...
- On choisi un(ou plusieurs) thème(s) - les thèmes pourraient être équivalents à différents ministères :
...
*** Je propose aussi : que l'on puisse signaler son désacord avec le préambule en signant par email. 
  ...
  n'ont que très peu de chance de se produire. Seule une nouvelle constitution pourra créer l'élan suffisant pour s'attaquer à l'ampleur de la tâche.
