...
* DEV
  ...
  ** @Tib et @Tango 
    ...
    *** repair DDA + bugs
      ...
      [ ] proposition
        [ ] create proposal
        [ ] Edit > shows > but doesn't save
        [ ] Deplacer
        [ ] translate dynform
        [ ] connect image
      [ ] rooms
        [ ] click once on actions, then again > 404
        [ ] Delete a room ??? can only Archive
      [ ] actions
        [ ] in actions room create an action > doesnt save 
        [ ] assign bug redirect
        [ ] connect image
      [ ] use directorys for room Lists
      [ ] virer les communes des drop down
      * test
        - creations on every type 
    *** usages
