...
>>>>>>>>[ ] check group existance before creation , otherwise just invite
          [ ] on demand : either we put a property on an element describing if corresponding exists or not 
          [ ] pre create all element's rooms 
>>>>>>>>[ ] only members, have access 
>>>>>>>>[ ] after create invite all members to chat and add notification
>>>>>>>>[ ] check anymember can start the chat
...
>>>>>>[[ ]] getting chat notifictions ???
        [[https://rocket.chat/docs/developer-guides/iframe-integration/events]]
      ** test 
        [ ] my chat shows fullscreen rocket with all activated channels
        [ ] an elements chat > show only the elements chat
        [ ] should members create new channels in their chat
...
>>>>>>>>>[ ] genéré tout les compte RC pour les comptes existant sinon ca peut planter les demandes chat direct
      ** bugs
        ...
        [ ] creating projects , name many words space seperated
  ...
  ** @Bouboule
  ...
  [X] Portefolio
    ...
    [ ] Merge collections inside portefolio
    [ ] addPhoto & addFile => can share as news
    [ ] add extract image on bookmark 
  [ ] Roles
    [ ] Open organizations to event only for admin
    [ ] Add roles (prefine and free) in addMultiMembers and in links (Database)
    [ ] List of role by contextType (json or ctk)
    [ ] Edit role in directory if admin
    [ ] filter roles (tags in directory) 
  ...
  [ ] idée 
    ...
    [ ] pour le 31/8 : sur un event ou une orga 
  ...
  [ ] Ajouter les roles en attribut des links
  ** @Rapha 
  [ ] internationalisation 
    [ ] Barcelone
