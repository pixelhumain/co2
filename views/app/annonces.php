	...
	<div class="col-lg-2 col-md-3 hidden-sm col-xs-12 padding-20 text-right hidden-xs" id="sub-menu-left">
		...
		<?php 
				...
				foreach ($classified["sections"] as $key => $section) { ?>
					...
					<hr>
					<?php } ?>
					<button class="btn btn-default margin-bottom-5 margin-left-5 btn-select-type-anc btn-anc-color-<?php echo @$section["color"]; ?>" 
									data-type-anc="<?php echo @$section["key"]; ?>">
						<i class="fa fa-<?php echo @$section["icon"]; ?> hidden-xs"></i> <?php echo @$section["label"]; ?>
					</button>
					<br>	
		<?php } ?>
