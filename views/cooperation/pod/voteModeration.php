...
<div class="col-lg-12 col-md-12 col-sm-12 padding-top-15 padding-bottom-5" id="podVote">
	...
	<div class="col-lg-3 col-md-4 col-sm-4 text-center pull-right">
		<canvas id="pieVote"/>
	</div>
 	...
 	?>
		...
		<div class="col-lg-8 col-md-8 col-sm-8 text-center no-padding pull-left margin-top-5">
			...
			<div class="col-lg-4 col-md-4 col-sm-6 text-center pull-left margin-top-5">
				<?php if(@$proposal["status"] == "tovote" && $auth && (!$hasVote || @$proposal["voteCanChange"] == "true")){ ?>
					<button class="btn btn-send-vote btn-link btn-sm bg-<?php echo $value["bg-color"]; ?> tooltips"
							...
							data-vote-value="<?php echo $value["voteValue"]; ?>"><?php echo Yii::t("moderation", $key); ?>
					</button>
				<?php }else{ ?>
					<label class="col-lg-12 col-md-12 col-sm-12 badge padding-10 bg-<?php echo $value["bg-color"]; ?> tooltips"
						...
						<?php echo Yii::t("moderation", $key); ?>
					</label>
				<?php } ?>

			</div>
			<div class="col-lg-2 col-md-2 col-sm-2 text-center pull-left margin-top-10 tooltips"
			...
			<div class="col-lg-4 col-md-4 col-sm-4 text-center pull-left margin-top-10 hidden-sm hidden-xs tooltips"
	...
	<div class="col-lg-12 col-md-12 col-sm-12 pull-left majority-space">
		...
		<?php if(@$proposal["status"] != "amendable" && $auth){ ?>
			<?php if($hasVote!=false){ ?>
				<h4 class="no-margin col-lg-4 col-md-4 col-sm-5 text-center pull-left" 
					...
					<small><?php echo Yii::t("cooperation", "You did vote"); ?> 
		...
		<h4 class="pull-right text-right"> 
			<small class="majority">
				...
				<span class="letter-blue">
					<?php if(@$voteRes["up"] && @$voteRes["up"]["percent"] && $voteRes["up"]["percent"] > @$proposal["majority"] ){ ?>
						 <?php echo Yii::t("moderation", "Currently, this message will be"); ?> 
						 <?php //if($proposal["status"] == "tovote"){ ?><?php //echo Yii::t("moderation", "currently"); ?> <?php //} ?>
						 <span class="bold letter-green"><?php echo Yii::t("moderation", "deleted"); ?></span>
					<?php }else{ ?>
						 <?php echo Yii::t("moderation", "Currently, this message will be"); ?> 
						 <?php //if($proposal["status"] == "tovote"){ ?><?php //echo Yii::t("moderation", "currently"); ?> <?php //} ?> 
						 <span class="bold letter-red"><?php echo Yii::t("moderation", "rehabilitated"); ?></span>
					<?php } ?>
				</span>
