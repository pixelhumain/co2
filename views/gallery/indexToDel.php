...
<script type="text/javascript">
	...
	function appendLevel(breadcrumLevel, name, buildNew){
		if(breadcrumLevel==0){
			if(docType=="image")
				nameFirst=trad.gallery;
			else
				nameFirst=trad.library;
			$html='<i class="fa fa-home fa-1x" style="padding: 0px 0px 0px 10px;" data-value="'+breadcrumLevel+'"></i><a href="javascript:;" onclick="galleryGuide('+breadcrumLevel+')" class="breadcrumAnchor bold" data-value="'+breadcrumLevel+'" data-name="'+name+'">'+nameFirst+'</a>';
		} else {
			nameBread=name;
			//if(nameBread=="")
			//	nameBread=trad[contentKey];
			$html= '<i class="text-red breadcrumChevron" style="padding: 0px 10px 0px 10px;" data-value="'+breadcrumLevel+'">/</i>'+'<a href="javascript:;" onclick="galleryGuide('+breadcrumLevel+',\''+name+'\',\''+docType+'\',\''+contentKey+'\', )" class="breadcrumAnchor bold text-dark" data-value="'+breadcrumLevel+'" data-name="'+nameBread+'">'+nameBread+'</a>';
		}
	...
	function openDirectory(folderId){
		if(typeof folderId != "undefined"){
			childrenFolder=getChildrenFolder(folderId);
		}
	...
	function bindButtonGalleryEvent(){
		$(".openFolder").off().on("click",function(){
			...
			breadcrumLevel++;
			appendLevel(breadcrumLevel,newCollection);
			getViewGallery();
		});
	...
	function buildNewBreadcrum(docType){
		...
		if(docType=="image"){
			breadcrumLevel++;
			appendLevel(breadcrumLevel,"album");
		}else if(docType=="files" || docType=="bookmarks"){
			breadcrumLevel++;
			appendLevel(breadcrumLevel,"file",docType,true);
		}
		$.each(navCollections, function(i,v){
			breadcrumLevel++;
